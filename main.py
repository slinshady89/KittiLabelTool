...
while i < len(consts.image_names) - 1:
    ...
    while j < i + k:
        ...
        if uvw_l[2] != 0:
            ...
            if pt_l[0] > 0 and pt_l[0] < width and pt_l[1] > 0 and pt_l[1] < height:
                left_in = 1
                cv2.circle(labeled_image, pt_l, 3, (0, 0, 255), -1)
            if pt_r[0] > 0 and pt_r[0] < width and pt_r[1] > 0 and pt_r[1] < height:
                right_in = 1
    ...
    if l > 1:
        while l < i + k:
            ...
            if l > len(consts.image_names):
                break
            I = np.eye(4)
            pose_chunk[:3, :4] = np.array(consts.poses[l]).reshape(3, 4)
            I = pose_chunk
            pose_chunk = np.matmul(inv_image_pose, pose_chunk)
            inv_image_pose = np.linalg.inv(I)
            # reshaping pt to 4x1
